...
import 'file:///G:/AndroidStudioProjects/bazar/juna/lib/screens/select_file.dart';
  ...
  static Future<List<FileSystemEntity>> searchFiles(String query,
    ...
    for (Directory dir in storage) {
      ...
      for (FileSystemEntity fs in fs) {
        if (basename(fs.path).toLowerCase().contains(query.toLowerCase()) && getExtention(fs.path) == ".pdf") {
          files.add(fs);
        }
      }
    }
